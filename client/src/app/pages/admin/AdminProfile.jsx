...
export default function AdminProfile() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        setProfile(response.data.admin_profile);
        setFormData({ phone_number: response.data.admin_profile?.phone_number || '' });
        setLoading(false);
      } catch (error) {
  ...
  const handleUpdate = async (e) => {
    ...
    setError('');
    try {
      await API.patch(`/users/admin_profiles/${profile.id}/`, formData);
      setProfile({ ...profile, ...formData });
      setShowEdit(false);
    } catch (error) {
      setError(error.response?.data?.phone_number?.[0] || 'Failed to update profile');
    }
  };
