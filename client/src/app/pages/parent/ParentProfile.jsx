...
export default function ParentProfile() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        setProfile(response.data.parent_profile);
        setFormData({
          phone_number: response.data.parent_profile?.phone_number || '',
          address: response.data.parent_profile?.address || ''
        });
        setLoading(false);
      } catch (error) {
  ...
  const handleUpdate = async (e) => {
    ...
    setError('');
    try {
      await API.patch(`/users/parent_profiles/${profile.id}/`, formData);
      setProfile({ ...profile, ...formData });
      setShowEdit(false);
    } catch (error) {
      setError(error.response?.data?.phone_number?.[0] || error.response?.data?.address?.[0] || 'Failed to update profile');
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-md p-6 max-w-2xl">
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-bold">{profile?.address}</span>
          </div>
        </div>
      </div>
